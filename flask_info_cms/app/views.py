...
# 查找数据
@app.route('/todo/api/v1/find/<string:username>', methods=['GET'])
def find(username):
    user = mongo.db.users
    ...
    return 'name:%s,password:%s'%(username, someone['password'])
...
def delete(username):
    ...
    if someone is not None:
        user.remove(someone)
        return '%s success to be removed'%(username)

    return abort(404)

#@app.route('/todo/api/v1/test')
#def get_data():
#    online_users = mongo.users.find({})
